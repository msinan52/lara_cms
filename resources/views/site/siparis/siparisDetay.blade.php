    ...
    <div class="bg-content">
        ...
        <table class="table table-bordererd table-hover">
            ...
            @foreach($order->basket->basket_items as $item)
                <tr>
                    <td><a href="{{ route('productDetail',$item->product->slug) }}">
                            <img src="{{config('constants.image_paths.product_image_folder_path').''.$item->product->image}}" class="img-responsive" height="100px"
                                 width="100px"></a></td>
                    <td><a href="{{ route('productDetail',$item->product->slug) }}">{{ $item->product->title }}</a></td>
                    <td>{{ $item->price }} ₺</td>
                    <td>{{ $item->qty }}</td>
            ...
            @if($basketCoupon)
                <tr>
                    <th colspan="5" class="text-right" style="color: green">Kupon {{ $basketCoupon->name }}</th>
                    <td class="text-right" style="color: green">- {{ $basketCoupon->discount_price }} ₺</td>
                </tr>
            @endif
            <tr>
