...
@section('header')
    ...
    <meta property="og:url" content="{{ route('categoryDetail',$category->slug)  }}"/>
    ...
    <link rel="canonical" href="{{ route('productDetail',$category->slug)  }}"/>
@endsection
@section('content')
    ...
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
        <div class="container">
            <ol class="breadcrumb">
                ...
                @if(!is_null($category->parent_cat->slug))
                    <li class="breadcrumb-item"><a
                            href="{{ route('categoryDetail',$category->parent_cat->slug != null ? $category->parent_cat->slug: '#' ) }}">{{ $category->parent_cat->title }}</a></li>
                @else
                ...
                <nav class="toolbox">
                    ...
                    <div class="toolbox-item toolbox-show">
                        <label><span class="spTotalProduct">{{ $data['productTotalCount'] }}</span> sonuçtan <span
                                class="spRangePageItems">0-{{ \App\Models\Urun::getPerPageStatic()  }}</span> gösteriliyor</label>
                    </div><!-- End .toolbox-item -->
                </nav>
                            ...
                            <div class="product-default">
                                ...
                                <div class="product-details">
                                    <h2 class="product-title">
                                        <a href="{{ route('productDetail',$product->slug) }}" title="{{$product->title}}">{{str_limit($product->title,25)}}</a>
                                    </h2>
                                    <div class="price-box">
