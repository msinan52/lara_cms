                                ...
                                <div class="col-md-3  pull-right">
                                    <select name="status_filter" class="form-control" id="status_filter" onchange="document.getElementById('form').submit()">
                                        ...
                                        @foreach($filter_types as $filter)
                                            <option value="{{ $filter[0] }}" {{ str_slug($filter[0]) == request('status_filter') ? 'selected': '' }}> {{ $filter[1] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        @foreach($list as $l)
                            <tr>
                                ...
                                <td>{{ $l ->created_at }} </td>
                                <td><a href="{{ route('admin.order.delete',$l->id) }}" onclick="return confirm('Silmek istediğine emin misin ?')"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
